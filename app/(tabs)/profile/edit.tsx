...
const EditProfileScreen = () => {
  ...
  const [showSavedMessage, setShowSavedMessage] = useState(false); // nuevo estado
  ...
  const handleConfirm = () => {
    console.log('Cambios guardados (visual):', {
      username: newUsername,
      email: newEmail,
      image: newImage,
    });
    setShowConfirm(false);
    setShowSavedMessage(true); // mostrar cartel de éxito
  };
      ...
      <Modal visible={showSavedMessage} transparent animationType="fade">
        <View className="flex-1 justify-center items-center bg-black bg-opacity-50 px-10">
          <View className="bg-white rounded-xl p-6 w-full items-center">
            ...
            <TouchableOpacity
              onPress={() => setShowSavedMessage(false)}
