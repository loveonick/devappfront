    ...
    <>
      <ScrollView className="flex-1 bg-white px-4 py-6">
        ...
        <View className="flex-row flex-wrap mb-4 gap-2">
          <TouchableOpacity className={`px-3 py-1 rounded-full ${activeTab === 'mis-recetas' ? 'bg-colorboton' : 'bg-gray-200'}`} onPress={() => setActiveTab('mis-recetas')}>
            <Text className={`font-medium ${activeTab === 'mis-recetas' ? 'text-white' : 'text-black'}`}>Mis Recetas</Text>
          </TouchableOpacity>
          <TouchableOpacity className={`px-3 py-1 rounded-full ${activeTab === 'guardadas' ? 'bg-colorboton' : 'bg-gray-200'}`} onPress={() => setActiveTab('guardadas')}>
            <Text className={`font-medium ${activeTab === 'guardadas' ? 'text-white' : 'text-black'}`}>Guardadas</Text>
          </TouchableOpacity>
        </View>

        {displayedRecipes.map((item) => (
          <TouchableOpacity key={item.id} onPress={() => router.push({ pathname: '/recipes/[id]', params: { id: item.id } })} className="mb-4">
            <RecipeCard recipeId={item.id} imgsrc={{ uri: item.imageUri }} title={item.title} description={item.description} tags={item.tags} author={item.author} date={item.date.toString()} />
          </TouchableOpacity>
        ))}
      </ScrollView>
        ...
        <View className="flex-1 justify-center items-center bg-black bg-opacity-50 px-10">
          <View className="bg-white rounded-xl p-6 w-full items-center">
            ...
            <View className="flex-row space-x-4">
              <TouchableOpacity onPress={() => setShowLogoutConfirm(false)} className="px-4 py-2 bg-gray-300 rounded-md">
                <Text className="text-black">No</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleConfirmLogout} className="px-4 py-2 bg-colorboton rounded-md">
